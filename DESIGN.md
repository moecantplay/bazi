...
**Glyph icons (the visual anchors):** elements are a bespoke 24×24 fine-line set — leaf / flame / mountain / square-holed coin / droplet, 1.7 stroke, round caps (`lib/glyph-icon-paths.ts`); the twelve zodiac animals are filled single-path silhouettes, uniform left-facing on a shared baseline (`lib/animal-icon-paths.ts`; AI-generated silhouettes vector-traced and geometrically polished — provenance in the file header). Both render via `components/glyph-icon.tsx` wherever a stem or branch shows: pillar grids (in 48px `bg-ink-tint` circles), Cycles timeline/decade/annual rows, horizon headers, finder results, the Today orbit. Element icons are **solid when yang, outlined when yin** — polarity you can see; animals are always solid, so the chart legend reads “element icon solid = yang · outlined = yin”. The dragon carries a second, more detailed path used at renders ≥40px (`renderSize`); the compact default is what survives chip sizes. Icons default to their element hue (element-fill rule above, 3:1 on paper and `--surface`); inside the hero wash they render ink. An icon never appears without its name in words beside or beneath it, and every icon carries an aria-label + hover title.
