...
**Glyph icons (English-first anchors):** a bespoke 24×24 set — five elements (leaf / flame / mountain / square-holed coin / droplet) and twelve zodiac animals — drawn in the orbit's fine-line language (1.7 stroke, round caps; `lib/glyph-icon-paths.ts`, rendered by `components/glyph-icon.tsx`). They stand where Han characters stand when Chinese characters are off: pillar grids, Cycles timeline/decade/annual rows, horizon headers, finder results, the Today orbit. Element icons are **solid when yang, outlined when yin** — polarity you can see; the chart grid carries the one-line legend ("solid symbol = yang · outlined = yin"). Icons default to their element hue (this falls under the element-fill rule above, 3:1 on paper and `--surface`); inside the hero wash they render ink (same-hue-on-wash rule as the retired swatch dots). An icon never appears without its name in words beside or beneath it, and every icon carries an aria-label + hover title. With Chinese characters on, the characters remain the anchors — icons don't double up.
