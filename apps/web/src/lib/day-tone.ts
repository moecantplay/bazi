/**
 * The week strip's at-a-glance tone for one day: the sum of every modelled
 * activity's score for the profile on that date, collapsed to a direction.
 * Pure engine math (no content layer), so the strip renders even before the
 * reading banks land. Kept to the ink/paper vocabulary — never a traffic light.
 */
