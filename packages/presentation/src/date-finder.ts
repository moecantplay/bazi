...
import { chartFor, chartForBirth } from "./chart.js";
...
/** Chart display names for the results list, primary profile first. */
export function dateFinderChartLabels(companionName: string | null): string[] {
  return companionName ? ["You", companionName] : ["You"];
}

/**
 * Base seed for the top-ranked day's verdict line; the caller appends
 * `|${date}` per candidate so every day's verdict is independently seeded.
 */
export function dateFinderVerdictSeedBase(
  profile: StoredProfile,
  activity: ActivityKey | null,
  companionId: string | null
): string {
  return `${natalSeedKey(profile)}|verdict|${activity ?? ""}${companionId ? `|${companionId}` : ""}`;
}
